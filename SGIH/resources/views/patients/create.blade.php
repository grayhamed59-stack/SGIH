<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 tracking-tight">Ajouter un patient</h1>
                <p class="text-sm text-gray-500">Remplissez les informations pour enregistrer un nouveau patient malien.</p>
            </div>
            ...
            <form action="{{ route('patients.store') }}" method="POST" class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full" value="{{ old('phone') }}" placeholder="+223" />
                        <x-input-error class="mt-2" :messages="$errors->get('phone')" />
                    </div>

                    <!-- Statut -->
                    <div>
                        <x-input-label for="status" :value="__('Statut initial')" />
                        <select id="status" name="status" class="mt-1 block w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-xl shadow-sm text-gray-900 font-bold">
                            <option value="En attente" @selected(old('status') == 'En attente')>En attente</option>
                ...
                <!-- Adresse -->
                <div>
                    <x-input-label for="address" :value="__('Adresse (Quartier, Ville)')" />
                    <textarea id="address" name="address" rows="3" class="mt-1 block w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-xl shadow-sm text-gray-900">{{ old('address') }}</textarea>
